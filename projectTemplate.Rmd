...
```
在目標變量Quality的統計數據，中位數/median是**6**，均值**5.878**，得知大多數的評價，集中在評比6。Outlier是**3,8,9**可見兩極評價相比之下非常少，我們可以從boxplot, histogram得到驗證。
...
```
揮發度酸，有者懸殊很大的離群數outlier，對比品質少數走向兩極，可以猜測揮發度酸的outlier可能會影響白酒評價，不是優質，就是劣質。
```{r}
unit_var('citric.acid',50)
unit_var_log('citric.acid',40)
summary(wine$citric.acid)
```
citric.acid 檸檬酸，數據出現**0**，outlier 1.6600遠遠超過，均值0.3342，3rd percentile 0.3900。
...
```
Density密度，是一個分佈很平均的變量，從boxplot可以得知，以及極少數的outliers，數據分佈在0.994附近，水密度為1，某種程度上來說，酒裡面大致上都是水也是沒錯的。
```{r}
unit_var('pH',50)
unit_var_log('pH',40)
summary(wine$pH)
...
```
residual.sugar/殘餘糖，數據分佈很散亂，平均值5.2，最大值65.8，分佈上是一個嚴重
right-skewed，酒精也是一個right-skewed的變量，彼此之間是否存在collinear是一個可以探討議題。
...
### Description Two
高評價白酒，密度稍為低，酒精成分高，酒精密度0.789 kg/m3，當酒精密度高，意味含量多，水的密度是1.0，所以密度自然被酒精影響。
